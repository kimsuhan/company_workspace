...
- 보호 참조: active Project의 `logo_file_id`, active Workspace User의 `profile_image_file_id`, active Project에 속한 Todo/Project 문서 본문, Note 본문 안의 `/api/files/:id` 또는 `/api/files/:id/download`
...
- Workspace Users: `workspace_users.profile_image_file_id`가 `files.id`를 참조한다. API 응답은 UI 표시용 `profileImageUrl`도 함께 내려준다.
