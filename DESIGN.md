...
colors:
  ...
  warning: "#f4b400"
  success: "#0fa336"
  action-primary: "{colors.bmw-blue}"
  action-danger: "{colors.m-red}"
...
- Inline actions inside a text input share the input group's outer border. Do not add a second bordered button box inside the field.
...
- Reserve the full M tricolor stripe for brand-identity moments — wordmark accents, motorsport chrome, model badges. Use single-color blue/red action tokens only for product UI controls.
...
- Don't use the full M stripe as a button fill. The stripe is a divider / accent; action buttons use one token color at a time.
...
7. The M tricolor stripe is brand-identity-only; product UI actions use `{colors.action-primary}` and `{colors.action-danger}` as single-color tokens.
